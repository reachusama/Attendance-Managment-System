...
namespace Vp_Final_Project.SubStudentForm
{
    public partial class StudentViewAttendance : Form
    {
        ...
        public StudentViewAttendance(int id)
        {
            InitializeComponent();
            userId = id;
            mysql = new MySql();

            comboBox_Course.DataSource = mysql.getAttendanceCoursesForStudent(userId);
            comboBox_Course.DisplayMember = "course_Name";
            comboBox_Course.SelectedItem = null;
            comboBox_Course.Text = "Select Course";

            comboBox_Date.DataSource = mysql.getAttendanceDatesForStudent(userId);
        ...
        {
            DataTable dt = mysql.getAttendanceByCourseAndDateForStudent(userId, comboBox_Course.Text, comboBox_Date.Text);
